...
void move_particles(array particle, array particle_v, const f32 dt, u64 n)
{
  ...
  for (u64 i = 0; i < n; i++)
  {
    ...
    {
  	  //Newton's law
      const f32 dx = particle.x[j] - particle.x[i]; //1
      const f32 dy = particle.y[j] - particle.y[i]; //2
      const f32 dz = particle.z[j] - particle.z[i]; //3
    ...
    //
    particle_v.x[i] += dt * fx; //19
    particle_v.y[i] += dt * fy; //21
    particle_v.z[i] += dt * fz; //23
  }

  ...
  for (u64 i = 0; i < n; i++)
  {
    ...
    for (u64 j = 0; j < n; j++)
    {
      ...
      const f32 d_3_over_2 = d_1_over_2 * d_1_over_2 * d_1_over_2; //24
  	  ...
  	  //Net force
  	  fx += dx * (1 / d_3_over_2); //27
  	  fy += dy * (1 / d_3_over_2); //30
  	  fz += dz * (1 / d_3_over_2); //33
  	}
    //
    particle_v.x[i] += dt * fx; //35
    particle_v.y[i] += dt * fy; //37
    particle_v.z[i] += dt * fz; //39
   
    particle.x[i] += dt * particle_v.x[i]; //41
    particle.y[i] += dt * particle_v.y[i]; //43
    particle.z[i] += dt * particle_v.z[i]; //45
  }

...
void move_particles(array particle, array particle_v, const f32 dt, u64 n)
{
  ...
  for (u64 i = 0; i < n; i++)
  {
    ...
    for (u64 j = 0; j < n; j++)
    {
      ...
      const f32 d_1_over_2 = sqrt(d_2); //10
      const f32 d_3_over_2 = d_1_over_2 * d_1_over_2 * d_1_over_2; //12
    ...
    //
    particle_v.x[i] += dt * fx; //20
    particle_v.y[i] += dt * fy; //22
    particle_v.z[i] += dt * fz; //24
    
    particle.x[i] += dt * particle_v.x[i]; //26
    particle.y[i] += dt * particle_v.y[i]; //28
    particle.z[i] += dt * particle_v.z[i]; //30
  }

...
void init(array particle, array particle_v, u64 n)
{
  for (u64 i = 0; i < n; i++)
    {
      //
      const u64 r1 = (u64)rand();
      const u64 r2 = (u64)rand();
      const f32 sign = (r1 > r2) ? 1 : -1;

      const f32 r_max = (f32)1.0 / (f32)RAND_MAX;
      const f32 r_max_signed = r_max * sign;

      //
      particle.x[i] = (f32)rand() * r_max_signed;
      particle.y[i] = (f32)rand() * r_max;
      particle.z[i] = (f32)rand() * r_max_signed;

      //
      particle_v.x[i] = (f32)rand() * r_max;
      particle_v.y[i] = (f32)rand() * r_max_signed;
      particle_v.z[i] = (f32)rand() * r_max;
    }
}
...
void move_particles(array particle, array particle_v, const f32 dt, u64 n)
{
  ...
  for (u64 i = 0; i < n; i++)
  {
    ...
    for (u64 j = 0; j < n; j++)
    {
      ...
      const f32 d_3_over_2 = d_2 * d_1_over_2; //23

      const f32 d_div = 1.0 / d_3_over_2; //24
...
int main(int argc, char **argv)
{
  ...
  particle.x = aligned_alloc(64, sizeof(f32) * n);
  particle.y = aligned_alloc(64, sizeof(f32) * n);
  particle.z = aligned_alloc(64, sizeof(f32) * n);
  particle_v.x = aligned_alloc(64, sizeof(f32) * n);
  particle_v.y = aligned_alloc(64, sizeof(f32) * n);
  particle_v.z = aligned_alloc(64, sizeof(f32) * n);
